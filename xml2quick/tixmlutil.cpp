...
namespace TixmlUtil {
    ...
    std::string GetStringAttribute(const TiXmlElement* elem, std::string attributename)
    {
        ...
        if (attribute == NULL)
        {
            ...
            throw QException(elem->Row(), attributename + " attribute expected in " + elemname);
        }
    ...
    std::string GetStringAttributeLen(const TiXmlElement* elem, std::string attributename, int len)
    {
        ...
        {
            throw QException(elem->Row(), "value for attribute " + attributename + " must be " + std::to_string(len) + " characters.");
        }
        return std::string(attribute);
    }
    ...
    int GetIntAttribute(const TiXmlElement* elem, std::string attributename)
    {
        ...
        for (size_t i = 0; i < satt.size(); ++i)
        {
            if (!isdigit(satt[i]))
            {
                throw QException(elem->Row(), "invalid character '" + satt[i] + std::string("' in number in attribute ") + attributename);
            }
        }
    ...
    int GetZeroOneAttribute(const TiXmlElement* elem, std::string attributename)
    {
        ...
        {
            throw QException(elem->Row(), "invalid number in attribute " + attributename + " - must be zero or one.");
        }
        return satt[0] - '0';
    }
